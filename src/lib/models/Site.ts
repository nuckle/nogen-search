...
export class Site {
	...
	textExistsInQuery(url: string, text: string): boolean {
		...
		const decodedValue = decodeURIComponent(searchParamValue);

		const parts = decodedValue.split(' ').filter(Boolean);
		return parts.includes(text);
	}
	...
	addTextToQueryInUrl(
		...
		for (const part of parts) {
			if (!this.isFilter(part) || !filterArray?.includes(part)) {
				newParts.push(part);
			}
		}
...
export class DuckDuckGo extends Site {
	...
	disableAssistantInUrl(url: string): string {
		const newUrl = this.setCustomParamsInUrl(url, this.disableAssistantParam);

		return newUrl;
	}

	disableChatInUrl(url: string): string {
		const newUrl = this.setCustomParamsInUrl(url, this.disableChatParam);
...
export class Bing extends Site {
	...
	disableCopilot(): void {
		chrome.cookies.get(
			{ url: 'https://www.bing.com', name: 'SRCHHPGUSR' },
			function (cookie) {
				if (!cookie) {
					return;
				}

				let params = new URLSearchParams(cookie.value);
				const paramName = 'CHTRSP';

				if (!params.has(paramName) || !(params.get(paramName) === '3')) {
					params.set(paramName, '3');
				}
